...
export function AIPage() {
  ...
  useEffect(() => {
    ...
    setMessages([{ role: 'assistant', content: '¡Hola parce! Soy tu copiloto. ¿En qué te ayudo hoy?' }])
  }, [])
  ...
  const sendMessage = async () => {
    ...
    setLoading(true)
    try {
      const { data } = await aiApi.copilot(newMessages)
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              className={`max-w-[85%] px-4 py-3 rounded-card font-body text-sm leading-relaxed ${
                msg.role === 'user'
